...
def discover_nsw_queue(
    *, bodies: list[dict[str, Any]], base_url: str, db_path: Path, delay: float, output: Path
) -> None:
...
def _download_control(*, repo_id: str, token: str, relative_path: str, destination: Path) -> bool:
    try:
        remote = hf_hub_download(
            repo_id=repo_id, repo_type="dataset", filename=relative_path, token=token
        )
...
def _assert_private_dataset(*, repo_id: str, token: str) -> None:
    ...
    if not bool(getattr(info, "private", False)):
        raise RuntimeError(
            f"refusing to retain restricted AU payload in non-private dataset {repo_id}"
        )
...
def _upload_file(
    *, path: Path, repo_id: str, token: str, path_in_repo: str, message: str
) -> object:
...
def _source_receipt(tranche_dir: Path, *, index: int, request_ids: list[int]) -> dict[str, Any]:
    files = {
        path.relative_to(tranche_dir).as_posix(): {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        frame["publication_authorized"] is not False
        or frame["private_hf_retention_authorized"] is not True
    ):
    ...
    _download_control(
        ...
        token=args.hf_token,
        relative_path=f"{CONTROL_ROOT}/capture_state.json",
    ...
    if not selection_path.exists():
        catalog = discover_bodies_with_fallback(
            ...
            output_path=control / "bodies.json",
            provenance_path=control / "bodies.provenance.json",
        ...
        nsw_bodies = [
            ...
            if isinstance(body, dict) and _body_jurisdiction(body, rules) == "NSW"
        ]
        ...
        if len(rows) != request_total:
            raise RuntimeError(
                f"pinned AU-NSW frame requires exactly {request_total} discovered requests, found {len(rows)}"
            )
        selection_path.write_text(
            "".join(json.dumps(row, sort_keys=True) + "\n" for row in rows), encoding="utf-8"
        )
        state = {
            "schema": "fyi-archive.au-nsw-private-hf-state.v1",
            "selection_sha256": _sha256_text(selection_path),
            "completed_tranches": [],
        }
    ...
    for index in range(1, tranche_count + 1):
        ...
        summary = run_seed(
            requests=tranche_requests,
            ledger_path=tranche_dir / "ledger.jsonl",
            data_dir=tranche_dir / "data",
            derived_dir=tranche_dir / "derived" / "requests",
            dist_dir=tranche_dir / "dist",
            caps=SeedCaps(
                max_requests=len(tranche_requests),
        ...
        manifest = build_manifest(
            load_derived_records(tranche_dir / "data" / "raw" / "requests"),
        ...
        write_manifest_outputs(
            ...
            parquet_path=tranche_dir / "manifest.parquet",
            authorities_path=tranche_dir / "authorities.json",
            instance_id="au-rtk",
        )
        _write_json(
            tranche_dir / "source_receipt.json",
            _source_receipt(tranche_dir, index=index, request_ids=request_ids),
        )
        payload_commit = publish_folder_to_hf(
            folder_path=tranche_dir,
            repo_id=args.hf_repo_id,
            token=args.hf_token,
            path_in_repo=f"{TRANCHE_ROOT}/tranche-{index:03d}",
            clean_stale=False,
            commit_message=f"data(au-nsw): retain tranche {index:03d}",
        )
        _write_json(
            tranche_dir / "upload_receipt.json",
            {
                "schema": "fyi-archive.huggingface-upload-receipt.v1",
                "tranche": index,
                "uploaded_at": _now(),
                "repo_id": args.hf_repo_id,
                "path": f"{TRANCHE_ROOT}/tranche-{index:03d}",
                "payload_commit": _commit_fields(payload_commit),
                "source_receipt_sha256": sha256_file(tranche_dir / "source_receipt.json"),
            },
        )
        _upload_file(
...
def main() -> int:
    ...
    parser.add_argument(
        "--rate-limit-db", type=Path, default=Path("data/_state/au-rtk/nsw-private-hf.db")
    )
