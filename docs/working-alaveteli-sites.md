...
Live operation is enabled once with the repository variable
`AUTONOMOUS_LIVE_CAPTURE_ENABLED=true`, after rate, rights, privacy, and
integrity readiness evidence has been recorded. It does not require a
per-dispatch confirmation. Live capture is restricted to each country's local
overnight hours, caps each site at five
...
complete capture-version coverage or successful live capture. The monthly
...
empirical freeze. A later dispatch can supply the failed
